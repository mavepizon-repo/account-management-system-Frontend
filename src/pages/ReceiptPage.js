...
function ReceiptPage({ onLogout }) {
  ...
  // ── KEY FIX: fetch receipts then enrich with invoices immediately ──
  const fetchAll = useCallback(async () => {
    try {
      ...
      // ✅ KEY FIX: Enrich each receipt with full invoice & client objects
      // In case backend only returns IDs instead of populated objects
      const enriched = rcptList.map(r => {
        // Resolve invoice
        const invoiceId = r.invoice?._id || (typeof r.invoice === 'string' ? r.invoice : null);
        const invoiceObj = invoiceId
          ? (invList.find(i => i._id === invoiceId) || r.invoice)
          : r.invoice;

        // Resolve client
        const clientId = r.client?._id || (typeof r.client === 'string' ? r.client : null);
        const clientObj = clientId
          ? (cliList.find(c => c._id === clientId) || r.client)
          : r.client;

        return {
          ...r,
          invoice: invoiceObj || r.invoice,
          client:  clientObj  || r.client,
        };
      });
  ...
  // ✅ FIX: Safe invoice number lookup — checks both populated object and invoices array
  const getInvoiceNumber = (r) => {
    if (!r) return '—';
    // If invoice is populated object
    if (r.invoice?.invoiceNumber) return r.invoice.invoiceNumber;
    // If invoice is just an ID string, look up in invoices array
  ...
  // ✅ FIX: Safe invoice object lookup
  const getInvoiceObj = (r) => {
    if (!r) return null;
    if (r.invoice?._id) return r.invoice;
    const invId = typeof r.invoice === 'string' ? r.invoice : null;
    return invId ? invoices.find(i => i._id === invId) : null;
  };
  ...
  // ── Build per-invoice summary from ALL receipts ──
  ...
  const filteredReceipts = receipts.filter(r => {
    ...
    // ✅ FIX: Use getInvoiceNumber helper for safe lookup
          ...
          <div className="panel-section" key="update">
            ...
            <div className="form-field" style={{ marginBottom: 20 }}>
              ...
              <SearchableDropdown
                ...
                getLabel={r => `${r.receiptNumber || '—'} — ${r.client?.name || '—'} — ${getInvoiceNumber(r)} — ₹${Number(r.amountPaid).toLocaleString('en-IN')}`}
                getId={r => r._id}
              />
            </div>
            ...
            {updateFound && (() => {
              ...
              const invObj  = getInvoiceObj(updateFound);
              return (
                <>
                  <div className="rcp-client-info-card" style={{ marginBottom: 20 }}>
                    <div className="rcp-client-info-row">
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        {/* ✅ FIX: Show invoice number safely */}
                        <div className="rcp-inv-project" style={{ marginTop: 4, fontSize: 12, color: '#7c3aed', fontWeight: 700 }}>
                          {getInvoiceNumber(updateFound)}
                        </div>
                        <div className="rcp-inv-project" style={{ marginTop: 4 }}>
                          {invObj?.subject || updateFound.invoice?.subject || '—'}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="rcp-balance-row" style={{ marginBottom: 20 }}>
                    <div className="rcp-balance-card rcp-bc-total">
                      ...
                      <strong>₹{Number(invObj?.grandTotal || updateFound.invoice?.grandTotal || 0).toLocaleString('en-IN')}</strong>
                    </div>
                    <div className="rcp-balance-card rcp-bc-paid">
        ...
        {activePanel === PANELS.DELETE && (
          <div className="panel-section" key="delete">
            ...
            {deleteFound && (() => {
              ...
              const invObj = getInvoiceObj(deleteFound);
              return (
                <div className="detail-card" style={{ marginTop: 20 }}>
                  {[
                    ...
                    // ✅ FIX: invoice number shown correctly
                    ['Invoice No',    getInvoiceNumber(deleteFound)],
                    ['Invoice Total', `₹${Number(invObj?.grandTotal || deleteFound.invoice?.grandTotal || 0).toLocaleString('en-IN')}`],
            ...
            {filteredReceipts.length === 0 ? (
              ...
              <>
                ...
                {selectedReceiptObj && (() => {
                  ...
                  const invObj     = getInvoiceObj(selectedReceiptObj);
                  const grandTotal = Number(invObj?.grandTotal || selectedReceiptObj.invoice?.grandTotal || 0);
                    ...
                    <div className="rcp-detail-card">
                      <div className="rcp-detail-header">
                        <div>
                          ...
                          {/* ✅ FIX: Show invoice number in header */}
                          <span className="inv-number-tag" style={{ marginRight: 10, background: '#f3e8ff', color: '#7c3aed', borderColor: '#ddd6fe' }}>{invNo}</span>
                          <span style={{ fontWeight: 700 }}>{invObj?.subject || selectedReceiptObj.invoice?.subject || '—'}</span>
                        </div>
                      ...
                      <div className="rcp-client-info-card" style={{ marginBottom: 14 }}>
                        <div className="rcp-client-info-row">
                          ...
                          <div style={{ textAlign: 'right' }}>
                            {/* ✅ FIX: invoice number prominently */}
                          ...
                          <div className="inv-detail-item" key={k}
                            ...
                            <span className="inv-detail-val"
                              style={
                                ...
                                k === 'Invoice No' ? { color: '#7c3aed', fontWeight: 700 } : {}
                              }>
                              {v}
                            </span>
                          </div>
                ...
                <div className="rcp-card-list">
                  {filteredReceipts.map(r => {
                    ...
                    const invNo   = getInvoiceNumber(r); // ✅ FIX
                    ...
                    return (
                      <div key={r._id}
                        ...
                        <div className="rcp-row-left">
                          ...
                          <div className="rcp-row-project">{r.invoice?.subject || invNo || '—'}</div>
                        ...
                        <div className="rcp-row-mid">
                          <div className="rcp-row-inv">
                            {/* ✅ FIX: Always shows invoice number */}
                            <span className="inv-number-tag">{invNo}</span>
                          </div>
