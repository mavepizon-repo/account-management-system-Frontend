    ...
    <div className="form-row invoice-form-grid">
      <div className="form-field">
        ...
        <div className="dropdown-wrap">
          <select className="dropdown-select" value={form.clientCode}
...
/* ── CSV export ──────────────────────────────────────── */
function exportInvoicesToExcel(invoices, getClientLabel, getInvNo, getSubject, allReceiptsSummary) {
  const headers = [
    'Invoice No','Date','Client','Subject',
    'Total Amount (₹)','Total GST (₹)','Grand Total (₹)','Paid Amount (₹)','Balance (₹)','Advance (₹)',
    'Payment Status','Notes'
  ];
  const rows = invoices.map(inv => {
    const grandTotal = Number(inv.grandTotal || 0);
    const summary    = allReceiptsSummary[inv._id];
    const totalPaid  = summary ? summary.totalPaid : Number(inv.paidAmount || 0);
    const advance    = totalPaid > grandTotal ? totalPaid - grandTotal : 0;
    const balance    = advance > 0 ? 0 : Math.max(0, grandTotal - totalPaid);
    return [
      getInvNo(inv), inv.date?.split('T')[0]||'', getClientLabel(inv), getSubject(inv),
      inv.totalAmount||0, inv.totalGST||0, grandTotal, totalPaid, balance, advance,
...
function InvoicePage({ onLogout }) {
  ...
  // ── KEY FIX: Store ALL receipts and build a per-invoice summary ──
  // allReceiptsSummary[invoiceId] = { totalPaid, grandTotal }
  // This is the single source of truth for advance amount calculation.
  ...
  /* ─────────────────────────────────────────────────────────
     KEY FIX: Fetch ALL receipts once and build a summary map.
     allReceiptsSummary[invoiceId] = { totalPaid, grandTotal }
     This gives us accurate advance amount for EVERY invoice
     without needing per-invoice receipt fetches.
  ───────────────────────────────────────────────────────── */
  const fetchAllReceiptsSummary = async () => {
    try {
      const r = await fetch(`${API_BASE_URL}/receipt/getall`);
      const d = await r.json();
      const receipts = d.receipts || [];

      // Build summary: group by invoice ID, sum amountPaid
      const summary = receipts.reduce((acc, receipt) => {
        const invId = receipt.invoice?._id || (typeof receipt.invoice === 'string' ? receipt.invoice : null);
        if (!invId) return acc;
        if (!acc[invId]) {
          acc[invId] = {
            totalPaid:  0,
            grandTotal: Number(receipt.invoice?.grandTotal || 0),
          };
        }
        acc[invId].totalPaid += Number(receipt.amountPaid || 0);
        return acc;
      }, {});

      setAllReceiptsSummary(summary);
    } catch {
      // Non-critical — advance amounts just won't show
    }
  };
  ...
  useEffect(() => {
    fetchClients();
    fetchInvoices();
    fetchAllReceiptsSummary();
  }, []);

  /* ─────────────────────────────────────────────────────────
     KEY FIX: getAdvanceAmount now uses allReceiptsSummary
     which is pre-loaded — no more "-1" unknown state.
     Returns: advance > 0 if overpaid, 0 otherwise.
  ───────────────────────────────────────────────────────── */
  const getAdvanceAmount = useCallback((inv) => {
    const grandTotal = Number(inv.grandTotal || 0);
    const summary    = allReceiptsSummary[inv._id];

    if (summary) {
      const advance = summary.totalPaid - grandTotal;
      return advance > 0 ? advance : 0;
    }

    // Fallback: no receipts at all for this invoice
    return 0;
  }, [allReceiptsSummary]);

  /* ─────────────────────────────────────────────────────────
     Fetch receipts for a specific invoice (for "View Receipts" toggle).
  ───────────────────────────────────────────────────────── */
  const fetchReceiptsForInvoice = useCallback(async (invId) => {
    if (invoiceReceipts[invId] !== undefined) return;
    try {
      ...
      setInvoiceReceipts(prev => ({
        ...prev,
        [invId]: Array.isArray(data) ? data : (data.receipts || []),
      }));
    } catch {}
  }, [invoiceReceipts]);
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      if (!res.ok) { const d=await res.json(); showToast(d.message||'Failed to update invoice','error'); return; }
      ...
      if (!productsRes.ok) { const d=await productsRes.json(); showToast(d.message||'Failed to update products','error'); return; }
      await fetchInvoices();
      await fetchAllReceiptsSummary(); // refresh advance summary after update
  ...
  /* ── DELETE ── */
  const handleDeleteSelect = (invId) => {
    setDeleteInvoiceId(invId);
    setDeleteFound(invoices.find(i=>i._id===invId)||null);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        await fetchInvoices();
        await fetchAllReceiptsSummary(); // refresh after delete
        showToast('Invoice deleted successfully!','info');
        setDeleteFound(null); setDeleteInvoiceId('');
  ...
  /* ── Inline edit ── */
  const startInlineEdit = inv => { setEditingId(inv._id); setInlineForm(invoiceToUpdateForm(inv)); };

  const saveInlineEdit = async (id) => {
    try {
      setLoading(true);
      const payload = buildUpdatePayload(inlineForm);
      ...
      await fetchInvoices();
      await fetchAllReceiptsSummary(); // refresh advance summary
  ...
  const handlePrint = (inv) => {
    ...
    // Use allReceiptsSummary for accurate paid amount and advance
    const summary        = allReceiptsSummary[inv._id];
    const totalReceiptPaid = summary ? summary.totalPaid : Number(inv.paidAmount || 0);
    const advance        = totalReceiptPaid > grandTotal ? totalReceiptPaid - grandTotal : 0;
    const balance        = advance > 0 ? 0 : Math.max(0, grandTotal - totalReceiptPaid);
    // Display paid = grandTotal when fully paid or advance
    const displayPaid    = totalReceiptPaid >= grandTotal ? grandTotal : totalReceiptPaid;
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .row-advance td{background:#f0fff4;color:#036b4e;font-weight:700;}
  ...
  <div class="body">
    ...
    <div class="totals-wrap">
      <table class="tot-tbl">
        ...
        <tr class="row-paid"><td class="tot-lbl">Paid Amount</td><td class="tot-val">&#8377;${fmt(displayPaid)}</td></tr>
        ${balance > 0 ? `<tr class="row-bal"><td class="tot-lbl">Balance Due</td><td class="tot-val">&#8377;${fmt(balance)}</td></tr>` : ''}
        ${advance > 0 ? `<tr class="row-advance"><td class="tot-lbl">Advance Credit</td><td class="tot-val">+&#8377;${fmt(advance)}</td></tr>` : ''}
      </table>
    </div>
...
<script>window.onload = () => { window.focus(); window.print(); };</script>
  ...
  /* ── Status badge — includes AdvancePayment ── */
  const statusBadge = (status) => {
    ...
    const label = status === 'AdvancePayment' ? 'Advance Paid' : status;
    return <span className={`status-badge ${map[status]||'status-pending'}`}>{label}</span>;
  };
  ...
  const renderTotalsSummary = (form) => {
    ...
    const adv  = paid > t.grandTotal ? paid - t.grandTotal : 0;
        ...
        {paid>0 && (
          <>
            <div className="inv-total-row">
              ...
              <strong style={{color:'#036b4e'}}>₹{Number(paid).toLocaleString('en-IN')}</strong>
            </div>
            {bal>0 && (
              <div className="inv-total-row">
                <span>Balance Due</span>
                <strong style={{color:'#c93360'}}>₹{Number(bal).toLocaleString('en-IN')}</strong>
              </div>
            )}
            {adv>0 && (
              <div className="inv-total-row inv-advance-row">
                <span>Advance Credit</span>
                <strong>+₹{Number(adv).toLocaleString('en-IN')}</strong>
              </div>
            )}
          </>
        )}
  ...
  const filteredInvoices = invoices.filter(inv => {
    const matchStatus  = statusFilter==='All'
      ...
      || (statusFilter==='Paid' && inv.paymentStatus==='AdvancePayment');
  ...
  const countPaid    = invoices.filter(i=>i.paymentStatus==='Paid'||i.paymentStatus==='AdvancePayment').length;
  ...
  /* ── Payment summary row for an invoice card ── */
  const renderPaymentSummaryRow = (inv) => {
    const grandTotal   = Number(inv.grandTotal || 0);
    const summary      = allReceiptsSummary[inv._id];
    const totalPaid    = summary ? summary.totalPaid : Number(inv.paidAmount || 0);
    const advance      = totalPaid > grandTotal ? totalPaid - grandTotal : 0;
    const displayPaid  = totalPaid >= grandTotal ? grandTotal : totalPaid;
    const balance      = advance > 0 ? 0 : Math.max(0, grandTotal - totalPaid);

    return (
      <div style={{display:'flex', gap:12, marginTop:4, flexWrap:'wrap', alignItems:'center'}}>
        <span className="inv-row-amt">₹{Number(grandTotal).toLocaleString('en-IN')}</span>
        <span className="inv-row-paid">Paid ₹{Number(displayPaid).toLocaleString('en-IN')}</span>
        {balance > 0 && <span className="inv-row-bal">Bal ₹{Number(balance).toLocaleString('en-IN')}</span>}
        {advance > 0 && (
          <span className="inv-row-advance">💰 Adv +₹{Number(advance).toLocaleString('en-IN')}</span>
        )}
        {(inv.totalGST||0)>0 && (
  ...
  const renderDetailPaymentGrid = (inv) => {
    ...
    const rows = [
      ...
      ['Paid Amount',   `₹${Number(displayPaid).toLocaleString('en-IN')}`],
    ];
    if (balance > 0) rows.push(['Balance Due', `₹${Number(balance).toLocaleString('en-IN')}`]);
    if (advance > 0) rows.push(['Advance Credit', `+₹${Number(advance).toLocaleString('en-IN')}`]);
    return rows;
  };
        ...
        {activePanel===PANELS.ADD && (
          <div className="panel-section" key="add">
            ...
            <form onSubmit={handleAdd}>
              <InvoiceAddHeaderFields form={addForm} onChange={(k,v)=>setAddForm(prev=>({...prev,[k]:v}))} clients={clients} />
              <div className="items-section-label">Work Items</div>
              <ItemsTable items={addForm.items} onChange={changeItems(setAddForm)} onAdd={addItem(setAddForm)} onRemove={removeItem(setAddForm)} />
            ...
            {updateFound && (
              <>
                ...
                <form onSubmit={handleUpdate}>
                  <InvoiceUpdateHeaderFields form={updateForm} onChange={(k,v)=>setUpdateForm(prev=>({...prev,[k]:v}))} clients={clients} />
                  <div className="items-section-label">Work Items</div>
                  <ItemsTable items={updateForm.items} onChange={changeItems(setUpdateForm)} onAdd={addItem(setUpdateForm)} onRemove={removeItem(setUpdateForm)} />
            ...
            {deleteFound && (
              <div className="detail-card" style={{marginTop:20}}>
                {[
                  ...
                  ['Grand Total',`₹${Number(deleteFound.grandTotal||0).toLocaleString('en-IN')}`],
          ...
          <div className="panel-section" key="getall">
            <div className="getall-header-row">
              ...
              <button className="excel-export-btn"
                onClick={()=>exportInvoicesToExcel(filteredInvoices,getClientLabel,getInvNo,getSubject,allReceiptsSummary)}
            ...
            <div className="inv-summary-chips">
              ...
              <div className="inv-chip-card chip-received">
                ...
                <strong>₹{Object.values(allReceiptsSummary).reduce((s,v)=>s+v.totalPaid,0).toLocaleString('en-IN')}</strong>
              </div>
              <div className="inv-chip-card chip-due">
                <span>Outstanding</span>
                <strong>₹{invoices.reduce((s,inv)=>{
                  const summary = allReceiptsSummary[inv._id];
                  const paid    = summary ? summary.totalPaid : Number(inv.paidAmount||0);
            ...
            {filteredInvoices.length===0
              ...
              : (
                <>
                  ...
                  {selectedInvoiceObj && (() => {
                    const advAmt     = getAdvanceAmount(selectedInvoiceObj);
                    const grandTotal = Number(selectedInvoiceObj.grandTotal || 0);
                    const prods      = Array.isArray(selectedInvoiceObj.products) ? selectedInvoiceObj.products : [];
                    const isAdv      = selectedInvoiceObj.paymentStatus === 'AdvancePayment';
                    const summary    = allReceiptsSummary[selectedInvoiceObj._id];
                    const totalPaid  = summary ? summary.totalPaid : Number(selectedInvoiceObj.paidAmount || 0);
                    const bal        = totalPaid >= grandTotal ? 0 : Math.max(0, grandTotal - totalPaid);
                      ...
                      <div className="inv-detail-card" style={{marginBottom:20}}>
                        <div className="inv-detail-header">
                          ...
                          <div style={{display:'flex',gap:8,alignItems:'center'}}>
                            ...
                            {advAmt > 0 && (
                              <span className="advance-tag">💰 Adv +₹{Number(advAmt).toLocaleString('en-IN')}</span>
                            )}
                        ...
                        <div className="inv-detail-grid">
                          {renderDetailPaymentGrid(selectedInvoiceObj).map(([k,v])=>(
                            <div className="inv-detail-item" key={k}
                              style={
                                k==='Balance Due'&&bal>0    ? {background:'#fff4f7', borderColor:'#ffc8d4'} :
                              ...
                              <span className="inv-detail-val"
                                style={
                                  k==='Balance Due'&&bal>0  ? {color:'#c93360'} :
                        ...
                        {/* Advance note banner */}
                        {isAdv && advAmt > 0 && (
                          <div className="advance-note-bar">
                            <span>💰</span>
                            <span>
                              This invoice has been <strong>fully paid</strong>. Client paid an extra{' '}
                              <strong>₹{Number(advAmt).toLocaleString('en-IN')}</strong> which is recorded as an{' '}
                          ...
                          <div className="receipt-subrow-inner" style={{marginTop:16}}>
                            {!invoiceReceipts[selectedInvoiceObj._id]||invoiceReceipts[selectedInvoiceObj._id].length===0
                              ...
                              : (() => {
                                  ...
                                  const totalRcptPaid = rcpts.reduce((s,r)=>s+(Number(r.amountPaid)||0),0);
                                  const rcptAdvance   = totalRcptPaid - grandTotal;
                                  return (
                                    <>
                                      {rcptAdvance > 0 && (
                                        <div className="advance-note-bar" style={{marginBottom:12}}>
                                          <span>💰</span>
                                          <span>
                                            Total receipts paid: <strong>₹{Number(totalRcptPaid).toLocaleString('en-IN')}</strong>.
                                            Invoice total: <strong>₹{Number(grandTotal).toLocaleString('en-IN')}</strong>.
                                            Advance Credit: <strong>₹{Number(rcptAdvance).toLocaleString('en-IN')}</strong>
                                          </span>
                                        </div>
                                      )}
                                      <table className="receipt-subtable">
                                        <thead>
                                          <tr><th>Receipt No</th><th>Amount Paid</th><th>Payment Date</th><th>PDF</th></tr>
                                        </thead>
                                        <tbody>
                                          {rcpts.map(r=>(
                                            <tr key={r._id}>
                                              <td><span className="client-code-tag" style={{background:'#f3e8ff',color:'#7c3aed',borderColor:'#ddd6fe'}}>{r.receiptNumber}</span></td>
                                              <td className="paid-cell">₹{Number(r.amountPaid).toLocaleString('en-IN')}</td>
                                              <td>{r.paymentDate?.split('T')[0]}</td>
                                              <td>{r.receiptPdf?<a href={r.receiptPdf} target="_blank" rel="noreferrer" className="inv-print-btn" style={{textDecoration:'none'}}>📄</a>:'—'}</td>
                                            </tr>
                                          ))}
                                        </tbody>
                                      </table>
                  ...
                  <div className="inv-card-list">
                    {filteredInvoices.map(inv => {
                      ...
                      const advAmt    = getAdvanceAmount(inv);
                      const isAdv     = inv.paymentStatus === 'AdvancePayment';
                      ...
                      return editingId===inv._id ? (
                        <div key={inv._id} className="inv-edit-card">
                          ...
                          <InvoiceUpdateHeaderFields form={inlineForm} onChange={(k,v)=>setInlineForm(prev=>({...prev,[k]:v}))} clients={clients}/>
                          <div className="items-section-label">Work Items</div>
                          <ItemsTable items={inlineForm.items} onChange={changeItems(setInlineForm)} onAdd={addItem(setInlineForm)} onRemove={removeItem(setInlineForm)}/>
                        ...
                        <div key={inv._id}
                          ...
                          <div className="inv-row-right">
                            <div style={{display:'flex', flexDirection:'column', alignItems:'flex-end', gap:4}}>
                              ...
                              {advAmt > 0 && (
                                <span className="advance-tag" style={{fontSize:10}}>
                                  💰 +₹{Number(advAmt).toLocaleString('en-IN')}
                                </span>
                              )}
                            </div>
